...
class RagProcessor {
    ...
    async process(filePath, docId, userId) {
        try {
            ...
            console.log(`[RAG] Extracted ${rawDocs.length} characters from PDF`);
            ...
            if (!vectors || vectors.length === 0 || vectors[0].length === 0) {
                throw new Error('Gemini API returned empty embeddings. Check your API key, model name, and quotas.');
            }
            ...
            await this.documentModel.updateStatus(docId, userId, DocumentModel.STATUS.INDEXED);
            console.log(`[RAG] Document ${docId} indexed successfully ✅`);

        } catch (err) {
            console.error(`[RAG] Processing failed for document ${docId}:`, err);
            // Mark document as failed
            await this.documentModel.updateStatus(docId, userId, DocumentModel.STATUS.FAILED);
        }
    }
